...
class Twitter {
  ...
  public:
    ...
    void postTweet(int userId, int tweetId) {
        tweets.push_back(new Tweet(tweetId, userId));
        return;
    }
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        if (follows.find(userId) != follows.end())
            followees = follows[userId];
        for (int i = tweets.size() - 1; i >= 0; i--) {
            Tweet *t = tweets[i];
            // 对于此条tweet，发布人是自己或是自己关注的列表则加入到feed列表
            if (t->userId == userId || followees.find(t->userId) != followees.end()) {
                res.push_back(t->id);
                if (res.size() == 10)
                    break;
            }
    ...
    void unfollow(int followerId, int followeeId) {
        auto followeesIt = follows.find(followerId);
        if (followeesIt != follows.end()) {
            followeesIt->second.erase(followeeId);
        }
    }
};
