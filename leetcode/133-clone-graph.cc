...
class Solution {
  public:
    ...
    Node *dfs(Node *n, unordered_map<Node *, Node *> &lookup) {
        if (n == nullptr)
            return nullptr;
        ...
        for (auto neighbor : n->neighbors) {
            newNode->neighbors.push_back(dfs(neighbor, lookup));
        }
        return newNode;
    }
};
        ...
        while (!q.empty()) {
            ...
            for (auto neighbor : n->neighbors) {
                if (lookup.find(neighbor) == lookup.end()) {
                    lookup[neighbor] = new Node(neighbor->val);
                    q.push(neighbor);
                }
                lookup[n]->neighbors.push_back(lookup[neighbor]);
            }
        }
